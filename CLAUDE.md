...
### `giacenze.js`
Backs the "Giacenze" view (`giacenze`): physical stock count vs. the expected stock imported from the management system. Tables `wt_giacenze_attese` (expected units, replaced wholesale on each import by a SuperAdmin), `wt_giacenze_conteggio` (count sessions, `stato` = `in_corso`/`completato`/`abbandonato`) and `wt_giacenze_conteggio_righe` (counted units per session+codice), plus two barcode maps: `wt_giacenze_ean` (authoritative EAN→codice, filled only by the CSV import) and `wt_giacenze_barcode_map` (barcode→codice links made by hand in the field; never write those into `wt_giacenze_ean`). The schemas **and** the atomic-increment RPC `wt_giacenze_incrementa` live in `tools/giacenze-schema.sql` and `tools/giacenze-ean-schema.sql` and must be run manually in the Supabase SQL editor (the anon key can't do DDL). The import is a real CSV parsed with PapaParse (CDN, pinned): each row is ONE physical unit; only `Stato = GIACENZA` rows count (units = rows per `Codice`), while the EAN map is built from every row that has a `Codice EAN`. Scan lookup order (`resolveBarcode()`): `wt_giacenze_ean` → `wt_giacenze_barcode_map` → barcode equal to a codice; an unknown barcode opens the manual search and Salva stores the link in `wt_giacenze_barcode_map`. Numeric barcodes are compared without leading zeros (`barcodeKey()`). The barcode scanner uses `html5-qrcode` (CDN, pinned). `formatsToSupport` goes in the **constructor** config, not `start()`. html5-qrcode decodes a canvas the size of the scan box in *layout pixels*, not camera pixels, so `#gzScannerBox` is deliberately 1000px wide and shrunk with a CSS `transform: scale()` (`fitScanner()`): at phone width the library sees ~300px and cannot read EAN-13 (95 modules), although QR works. Codes are always compared via `normKey()` (trim + uppercase) so case never matters. Excel export reuses the ExcelJS pattern from `parco-sim.js` (no shared helper exists).
